  ...
  <AuthenticatedLayout>
    <v-container fluid>
      <v-row>
        ...
        <v-col cols="12" md="4">
          <v-card class="customization-card" exportparts="5">
        ...
        <v-col cols="12" md="3" class="sliders-sticky" elevation="2">
...
const loadCustomizationData = (customizationData) => {
  ...
  // Converter elementos de string JSON para array
  if (customizationData.elements) {
    ...
    form.value.elements = parsedElements.map(element => {
      const baseElement = {
        ...
        top: element.top,
        left: element.left,
        width: element.width || form.value.input_width,
        height: element.height || form.value.input_height,
      ...
      if (element.type === 'input' || element.type === 'inputPassword') {
        return {
          ...
          shape: element.shape || form.value.input_shape
        };
      }
...
const loadElements = () => {
  if (customization && customization.elements) {
   ...
   // console.log('customization',form.value.id);
...
const saveScreenAsJson = () => {
  ...
  const method = isEditMode.value ? 'put' : 'post'; // 'put' para atualizar, 'post' para inserir
  ...
  .then(response => {
    console.log('Customização salva com sucesso!', response);
     window.location.href ='/login_customizations';
